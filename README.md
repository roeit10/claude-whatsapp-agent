...
הסוכן מותקן ב-`whatsapp-agent/` **בתוך התיקייה שאתם עובדים בה** — כל פרויקט
מקבל סוכן משלו, ואין התנגשות עם התקנות אחרות.
...
**הסודות נשארים אצלכם.** תיקיית הסוכן מקבלת `.gitignore` משלה שמחריג את
`.env` (טוקן Green API), `mcp.json` (מפתח Composio) ו-`logs/` — כך שגם אם
התקנתם בתוך ריפו גיט, שום דבר מזה לא נדחף.

**ההתקנה כוללת הפעלה אוטומטית.** ה-poller הוא תהליך רגיל — אם מפעילים אותו
מחלון טרמינל הוא מת עם החלון, בלי שום הודעה. לכן ההתקנה מגדירה אותו לעלות לבד.

**אחרי ריצה ארוכה — שלחו `/reset`.** השיחה תופחת והתשובות מאטות. איפוס מתחיל
שיחה נקייה בלי לאבד יכולות, חיבורים או קבצים שנוצרו.

**ריאקציות אמוג'י על ההודעה לא אפשריות.** ל-Green API אין מתודה כזו בשום תוכנית.
