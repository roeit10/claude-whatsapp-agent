...
## Composio מעל HTTP — תקלות

| תסמין | סיבה | פתרון |
|---|---|---|
| 401 מ-`connect.composio.dev` | מפתח שגוי, או שהשתמשת במפתח מ-`~/.composio/user_data.json` | המפתח חייב להיות מהדשבורד, מתחיל ב-`ck_` |
| "המערכת לא מחוברת" | המפתח תקין אבל Gmail/יומן לא חוברו | דשבורד → **Connect Apps** |
| `num_turns` = 1 | הכלים לא הורשו | `mcp__composio` ב-`--allowedTools` |
| תשובה חוזרת אבל ריקה | קריאה במבנה שגוי | ⚠️ קריאה שגויה מחזירה **HTTP 200 עם שגיאה בגוף** — אל תסתמך על קוד הסטטוס, קרא את התוכן |
