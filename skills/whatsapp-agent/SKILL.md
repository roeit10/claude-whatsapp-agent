...
**והעתק גם את `templates/agent.gitignore` בשם `.gitignore`** לתוך תיקיית הסוכן.
זה לא נוי: תיקיית הסוכן יושבת בתוך הפרויקט של המשתמש, ואם הפרויקט הזה הוא ריפו
גיט — `.env` (טוקן Green API) ו-`mcp.json` (מפתח Composio) ייכנסו לקומיט,
ואם הריפו ציבורי הם ידלפו. קובץ ה-`.gitignore` יושב **בתוך** תיקיית הסוכן ולכן
מגן עליה בלי תלות בהגדרות של הפרויקט שמעליה.

גם `logs/` מוחרג שם — הוא מכיל כל הודעה שהגיעה למספר.
...
**1. המשתמש מחבר אפליקציות ומביא מפתח**

שני הדברים נעשים בדפדפן. **הקרא לו את הצעדים אחד-אחד וחכה** — אלה פעולות
שרק הוא יכול לעשות, ואין דרך לעקוף אותן.

> תיכנס ל-https://dashboard.composio.dev ותתחבר (או תירשם, זה בחינם).
>
> **א. לחבר את המערכות שאתה רוצה שהסוכן יגיע אליהן:**
> בתפריט הימני **Connect Apps** → חפש `Gmail` → **Connect** → תאשר בחלון של גוגל.
> אותו דבר ל-`Google Calendar`. אפשר גם Monday, Airtable, מה שתרצה.
>
> **ב. להעתיק את המפתח:**
> בתפריט הימני **Settings** → **Sessions & API Key** → תחת "Your API Key"
> לחץ **Copy**. המפתח מתחיל ב-`ck_`.
>
> תשלח לי אותו כאן.

**סדר הפעולות חשוב.** אם הוא ייתן מפתח בלי לחבר קודם את Gmail והיומן,
האימות בשלב 3 ייכשל בהודעה על מערכת לא מחוברת — וזה ייראה כאילו המפתח שגוי.

אל תנסה להשיג את המפתח בעצמך: ה-CLI לא מייצר אותו, והמפתח ששמור מקומית
ב-`~/.composio/user_data.json` הוא מפתח סשן של ה-CLI ו**לא** יעבוד מול נתיב
ה-HTTP (מחזיר 401).
